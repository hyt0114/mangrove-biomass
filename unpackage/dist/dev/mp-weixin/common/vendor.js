...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__65283B8",
    appName: "mangrove-biomass",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function finalise(x, sd, rm, isTruncated) {
  var digits, i2, j2, k, rd, roundUp, w2, xd, xdi, Ctor = x.constructor;
  out:
    if (sd != null) {
      xd = x.d;
      if (!xd)
      ...
      (i2 > 0 ? j2 > 0 ? w2 / mathpow(10, digits - j2) : 0 : xd[xdi - 1]) % 10 & 1 || rm == (x.s < 0 ? 8 : 7));
      if (sd < 1 || !xd[0]) {
        xd.length = 0;
        if (roundUp) {
          sd -= x.e + 1;
      ...
      if (i2 == 0) {
        ...
        xd.length = xdi + 1;
        k = mathpow(10, LOG_BASE - i2);
        xd[xdi] = j2 > 0 ? (w2 / mathpow(10, digits - j2) % mathpow(10, j2) | 0) * k : 0;
      }
      if (roundUp) {
        for (; ; ) {
          if (xdi == 0) {
...
const pages = [
  {
    ...
    style: {
      navigationBarTitleText: "红树林-生物量测算"
    }
  }
];
const globalStyle = {
