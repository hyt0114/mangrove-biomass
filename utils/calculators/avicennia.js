...
export default class Avicennia {
	...
	calcByMacrophanerophytes() {
		const wa = new Decimal(10).pow(new Decimal(2.092).plus(new Decimal(0.529).times(Decimal.log10(
			new Decimal(this.#dbh).pow(2).times(this.#height)))));
		const wb = new Decimal(1.361).plus(new Decimal(0.615).times(Decimal.log10(new Decimal(this.#dbh).pow(
			2).times(this.#height))));
		return {
	...
	calcByShrub() {
		if (this.#crown && this.#crown > 0) {
			const wa = new Decimal(0.00801).times(new Decimal(this.#height).ln().times(
					this.#height)).minus(-2.4386)
				.plus(new Decimal(1.4796).times(this.#crown)).plus(new Decimal(this.#basal).pow(2).times(this
					.#basal));
